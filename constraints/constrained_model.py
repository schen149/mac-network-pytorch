...
class MACUnit(nn.Module):
    ...
    def forward(self, context, question, knowledge):
        ...
        for i in range(self.max_step):
            ...
            read, attn = self.read(memories, knowledge, controls)
            if i == int(self.max_step / 2):
                mid_layer_attn = attn.clone().detach()
...
class MACNetwork(nn.Module):
    ...
    def forward(self, image, question, question_len, dropout=0.15):
        ...
        memory, mid_layer_attn = self.mac(lstm_out, h, img)

        out = torch.cat([memory, h], 1)
        out = self.classifier(out)

        return out, mid_layer_attn
